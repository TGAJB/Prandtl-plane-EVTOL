...
# ----- Class 2 Propeller Mass Constants (empirical power‑law) -----
PROP_MASS_COEFF = 0.5       # [-]   Scaling constant for composite propellers
PROP_POWER_EXP  = 0.7       # [-]   Power exponent
PROP_DIAM_EXP   = 0.4       # [-]   Diameter exponent
...
def COMPUTE_MTOW(MTOW):
    ...
    # ---------------------------------------------------------------------
    # Class 2 propeller mass estimation (empirical power‑law)
    # ---------------------------------------------------------------------
    def propeller_mass(mtow_kg, d_prop, n_blades, n_props, max_power_kw_total,
                       coeff=PROP_MASS_COEFF, pow_exp=PROP_POWER_EXP,
                       diam_exp=PROP_DIAM_EXP):
        """
        Single‑propeller mass from empirical power‑law correlation.
        Inputs:
            max_power_kw_total : total shaft power in kW (from MAX_POWER)
        """
        p_per_prop_kw = max_power_kw_total / n_props          # [kW]
        m_prop_one = coeff * (p_per_prop_kw ** pow_exp) * (d_prop / 1.9) ** diam_exp
        m_prop_all = m_prop_one * n_props
        return m_prop_one, m_prop_all

    # MASS CALCULATIONS

    M_F = (0.453592) * (14.86 * ((MTOW * 2.20462) ** 0.144) * ((L_FUS * 3.28084) ** 0.778) / ((PER_FUS_MAX * 3.28084)) ** 0.778) * ((L_FUS * 3.28084) ** 0.383) * N_PAX ** 0.455
    ...
    # ---- Updated propeller mass (empirical) ----
    M_PROP, M_PROP_TOT = propeller_mass(
        MTOW, D_PROP, N_BLADES, N_PROP, MAX_POWER
    )
    ...
    print("Miscellaneous: " + str(M_MISC))
