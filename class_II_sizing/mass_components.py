...
def landing_gear_mass(mtow_kg, E, rho, sigma_allow=SIGMA_ALLOW_AL):
    ...
    (i)  Equal force split P = F_max / n_react is valid for a SYMMETRIC
         ...
         statically-indeterminate or 6-DOF dynamic solve.
    ...
    def _size_condition(Do, Vz, eta, m_total):
        ...
        Quadratic stroke solver (CS-27/29 energy requirement):
            0.5·K·δ² - (W - L)·δ - 0.5·m·Vz² = 0   (positive root)
        Cantilever root stress (symmetric drop, equal force split):
            σ = (F_max / n_react) · L_eff · (Do/2) / I
        """
        Di = Do - 2 * T_WALL_SKID
        if Di <= 0:
        ...
        W     = m_total * G
        L     = KAPPA_LG * W
        K     = N_REACT * 3 * E * I / L_EFF**3   # legs in parallel
        ...
        # Positive root of 0.5·K·δ² - (W−L)·δ - 0.5·m·Vz² = 0
        b_q   = -(W - L)
        delta = (-b_q + np.sqrt(b_q**2 + K * m_total * Vz**2)) / (2 * 0.5 * K)

        n     = K * delta / W
        sigma = (K * delta / N_REACT) * L_EFF * (Do / 2) / I
            ...
            for Do in Do_arr:
                ...
                if MS >= 0 and n <= N_LIMIT_LG and delta <= GROUND_CLEARANCE:
                    if name not in best or mg < best[name][0]:
                        best[name] = (mg, Do, MS, n, delta, name)
        if not best:
            return None
